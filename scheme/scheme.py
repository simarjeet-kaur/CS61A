...
def eval_all(expressions, env):
   ...
   # if expressions == nil:
    ...
    return eval_all(expressions.second, env) #eval_all(expressions.second, env))
    # END PROBLEM 8
...
class Frame(object):
    ...
    def __init__(self, parent):
        ...
        self.bindings = {} #dictionary representing the bindings in the frame
        #maps Scheme symbols (python strings) to Scheme values
        self.parent = parent #parent Frame instnace - parent of Global is None
    ...
    def make_child_frame(self, formals, vals):
        ...
        """
        #define method of a frame instance creates a binding in that frame
        # BEGIN PROBLEM 11
        new_frame = Frame(self)
        if len(vals) != len(formals):
            raise SchemeError
        else:
            def binding(formal, val):
                if formal != nil:
                    new_frame.define(formal.first, val.first)
                    binding(formal.second, val.second)
            binding(formals, vals)
        return new_frame
        # END PROBLEM 11
...
#builtin procedures are applied by calling a corresponding python function that 
#implements the procedure
# for example: + procedure is like add in Python
    def __init__(self, fn, use_env=False, name='builtin'):
        self.name = name
        self.fn = fn #python function that implements built in Scheme procedure
        self.use_env = use_env #boolean flag that idicates whether ot not this built in
        #procedure will expect the current env to be passed in as the last argument
        #env is required to implement the built in eval procedure`
    ...
    def apply(self, args, env):
        ...
        #takes in a list of arg values and the curr env - args is a Scheme list 
        #presented as a pair object
...
def do_and_form(expressions, env):
    ...
    # BEGIN PROBLEM 13
    if expressions is nil:
        return scheme_truep(True)
    elif expressions.second is nil:
        return scheme_eval(expressions.first, env)
    elif scheme_truep(scheme_eval(expressions.first, env)) is scheme_truep(False):
        return scheme_truep(False)
    elif expressions.second is not nil:
        return do_and_form(expressions.second, env)

    # elif expressions.second is not nil:
    #     if scheme_truep(scheme_eval(expressions.first, env)) and do_and_form(expressions.second, env):
    #         return scheme_eval(expressions.second, env) #needs to be the last one
    #     else:
    #         return scheme_truep(False)
    #elif expressions is nil:
     #   return scheme_truep(True)
    #return scheme_truep(False)
    #return scheme_eval(expressions.first, env)
    "*** YOUR CODE HERE ***"
...
def do_or_form(expressions, env):
    ...
    # if expressions is nil:
    #     return scheme_truep(False)
    # elif scheme_eval(expressions.first, env) is scheme_truep(False):
    ...
    if expressions.second is nil:
        ...
        return scheme_eval(expressions.first, env)
    else:# not scheme_truep(scheme_eval(expressions.first, env)):
