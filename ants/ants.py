    ...
    def remove_insect(self, insect):
        ...
        if insect.is_ant:
            ...
            if isinstance(insect, QueenAnt) and insect.is_queen:
                return 
                #self.ant = insect
            #elif not insect.is_queen and not isinstance(self, QueenAnt):
                #self.ant = None
            # END Problem 13
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        if self.place.ant is not None:
            return False or self.place.ant.blocks_path
        return False
        # END Problem 7
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        # BEGIN Problem 6
        if self.time_to_digest > 0 and self.digesting:
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        if not self.is_queen:
            ...
            #self.throw_at(self.nearest_bee(colony.hive))
            #buff ants BEHIND the queen by doubling their damage
            queen_place_exit = self.place.exit
            while queen_place_exit != None:
                ant = queen_place_exit.ant
                if ant not in self.buffed_ants and ant:
                    self.buffed_ants += [ant]
                    ant.damage = ant.damage * 2
                if ant and ant.is_container and ant.contained_ant and ant.contained_ant not in self.buffed_ants:
                    ant.contained_ant.damage = ant.contained_ant.damage * 2
                    self.buffed_ants += [ant.contained_ant]
                queen_place_exit = queen_place_exit.exit
